...
export default function ShowModal ({ isOpen, placement, size, handleClose, hideCloseButton, content }: ModalComposePostProps) {
  return (
    <Modal
      ...
      size={size}
      onClose={handleClose}
      classNames={{ base: ['bg-black min-h-72'], backdrop: ['bg-zinc-950/75'], body: ['px-0'], closeButton: ['left-1 w-fit hover:bg-white/10 transition-colors ease-in duration-150 text-white text-xl'] }}
    >
      <ModalContent>
        <>
