...
import { useCreateNewAccount, useStoreInputCredentials } from '@/hooks/useLogin'


export const HeaderForm = () => {

    const {setInitialForm} = useStoreInputCredentials(state => state)
    const {setInitialForm: setNewAccountForm} = useCreateNewAccount(state => state)

    const router = useRouter()
    const handleNavigationHome = () => {
        
        const initialFormChooseAccount = { user: "", password: ""}
